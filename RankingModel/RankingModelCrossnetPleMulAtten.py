...
# --- Training loop ---
def train(model, opt, loss_fn, dataset, best_model_path, epochs=5, batch_size=256,  device='cuda'):
    ...
    for ep in range(1, epochs+1):
        ...
        aucs = evaluate(model, dataset)  # Pass validation dataset
        avg_auc = sum(aucs) / len(aucs)

        if avg_auc > best_auc:
            best_auc = avg_auc
            # torch.save(model.state_dict(), best_model_path)
            save_checkpoint(model, optimizer=opt, epoch=5, path=best_model_path)
...
if __name__ == "__main__":
    ...
    best_model_path ='./Data/RM_CPM_model_best.pth'
    model = PLERecModel(D_user, D_item)
    lr=1e-4
    opt = torch.optim.Adam(model.parameters(), lr=lr, weight_decay=1e-5)
    loss_fn = nn.BCEWithLogitsLoss()
    train(model, opt, loss_fn, ds, best_model_path, epochs=5, batch_size=256,  device='cuda')

    # save_checkpoint(model, optimizer=opt, epoch=5, path='./Data/RM_CPM_model_epoch5.pth')
